...
Ans:When predicting to `ames_train`, the RMSE for `model.AIC` is higher than the RMSE for `model.full`.  However, when predicting to `ames_test`, the RMSE for `model.full` is higher.
...
```{r Q4}
k_BIC = log(nrow(ames_train))
model.BIC = stepAIC(model.full, k = k_BIC)
...
```{r Q5}
sqrt(mean((exp(predict(model.bas, newdata = ames_test, estimator="HPM")$fit) - ames_test$price)^2))
sqrt(mean((exp(predict(model.bas, newdata = ames_test, estimator="BPM")$fit) - ames_test$price)^2))
sqrt(mean((exp(predict(model.bas, newdata = ames_test, estimator="BMA")$fit) - ames_test$price)^2))
```
