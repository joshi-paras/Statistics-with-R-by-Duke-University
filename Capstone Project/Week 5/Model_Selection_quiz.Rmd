...
```{r Q1}
model_lm = lm(log(price)~ log(Lot.Area)+log(area)+Bedroom.AbvGr+Overall.Qual+Land.Slope, data = ames_train)
AIC_lm <- stepAIC(model_lm, k =2)
summary(AIC_lm)
BIC_lm <- stepAIC(model_lm, k = log(1000))
summary(BIC_lm)
```
...
Ans:Larger houses on average have more bedrooms and sell for higher prices.  However, holding constant the size of a house, the number of bedrooms decreases property valuation.
...
```{r Q3}
data.frame(Neighborhood = ames_train$Neighborhood, Residuals = resid(lm(log(price)~log(area),ames_train))) %>% group_by(Neighborhood) %>% summarise(meanRes = mean(Residuals)) %>% arrange(desc(meanRes))
...
```{r Q4}
data.frame(Neighborhood = ames_train$Neighborhood, Residuals = resid(lm(log(price)~log(area),ames_train))) %>% group_by(Neighborhood) %>% summarise(meanResSqr = mean(Residuals^2)) %>% arrange(desc(meanResSqr))
```
...
```{r Q5}
ames_train %>% filter(is.na(Overall.Qual) | is.na(Bsmt.Qual) | is.na(Garage.Qual)) %>% summarise(count = n())
...
Ans:Recode all `NA` values as a separate category, since houses without basements or garages are fundamentally different than houses with both basements and garages.
...
Ans: 075: 2-1/2 story houses

```{r Q7}
data.frame(MSSubClass = ames_train$MS.SubClass, predictedPrice = predict(lm(log(price)~Overall.Cond+Overall.Qual,ames_train), ames_train)) %>% group_by(MSSubClass) %>% summarise(medianMSSubClass = median(predictedPrice)) %>% arrange(desc(medianMSSubClass))
...
```{r Q8}
hat <- hatvalues(lm(log(price)~Overall.Cond+Overall.Qual,ames_train))
hat[c(125, 268, 640, 832)]
```
...
Ans: Holding constant all other variables in the model, on average, an additional bedroom will increase housing price by $k$ percent.
...
```{r Q11}
ames_train %>% filter(Sale.Condition == "Normal") %>% summarise(PercentRemain = n()/1000*100)
...
Ans: True, restricting the model to only include observations with normal sale conditions increases the $R^2$ from 0.547 to 0.575.

```{r Q12}
summary(lm(log(price)~log(area),ames_train))$r.squared
sapply(unique(as.character(ames_train$Sale.Condition)),
                 function(SaleCond)summary(lm(log(price)~log(area),ames_train,subset=(SaleCond==Sale.Condition)))$r.squared, simplify=FALSE,USE.NAMES=TRUE)
```
